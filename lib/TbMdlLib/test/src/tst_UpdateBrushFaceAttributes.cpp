...
TEST_CASE("UpdateBrushFaceAttributes")
{
  ...
  constexpr auto UvPls = UvSign::plus;
  constexpr auto UvMns = UvSign::minus;
    ...
    SECTION("Rectangular off-center face (-Y normal)")
    {
      const auto
        [mapFormat,
         ...
         axis,
         brushSize,
         expectedFitted] = GENERATE_COPY(values<T>({
      ...
      const auto
        [mapFormat,
         ...
         expectedScale] = GENERATE_COPY(values<T>({
          ...
          {Std, {0, 0}, {1.6, 1}, 0, UvU, UvPrev, {64, 64, 64}, {2, 0}},
          ...
          {Std, {0, 0}, {2, 1}, 0, UvU, UvBest, {48, 48, 48}, {1.5, 0}},
