...
TEST_CASE("Map_Brushes")
{
  ...
  SECTION("autoFitUV")
  {
    ...
    SECTION("Aligns using best policy when any selected face is not aligned")
    {
    ...
    SECTION(
      "Aligns using requested policy when selected faces are aligned and already fitted")
    {
      deselectAll(map);
      selectBrushFaces(map, {{brushNode, iFront}});
      REQUIRE(setBrushFaceAttributes(
      ...
      REQUIRE(getFace(*brushNode, iFront).uAxis() == vm::approx{vm::vec3d{1, 0, 0}});
      REQUIRE(getFace(*brushNode, iFront).vAxis() == vm::approx{vm::vec3d{0, 0, -1}});
      REQUIRE(getFace(*brushNode, iRight).uAxis() == vm::approx{vm::vec3d{0, 1, 0}});
      REQUIRE(getFace(*brushNode, iRight).vAxis() == vm::approx{vm::vec3d{0, 0, -1}});
      ...
      CHECK(getFace(*brushNode, iFront).uAxis() == vm::approx{vm::vec3d{0, 0, 1}});
      CHECK(getFace(*brushNode, iFront).vAxis() == vm::approx{vm::vec3d{1, 0, 0}});
      CHECK(getFace(*brushNode, iRight).uAxis() == vm::approx{vm::vec3d{0, 0, 1}});
      CHECK(getFace(*brushNode, iRight).vAxis() == vm::approx{vm::vec3d{0, 1, 0}});
    }
